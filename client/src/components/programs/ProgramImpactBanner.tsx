...
export function ProgramImpactBanner({ program, stats, cta, onShare }: ProgramImpactBannerProps) {
            ...
            {stats && stats.length > 0 ? (
              stats.map(({ label, value }) => (
                <div key={label} className="text-center bg-white/10 backdrop-blur-sm rounded-lg p-4">
                  <div className="text-2xl sm:text-3xl font-bold text-yellow-300 mb-1">{value}</div>
